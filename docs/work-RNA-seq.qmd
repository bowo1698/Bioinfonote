...
![AHPND pada udang vanamei yang menyebabkan kekosongan pada hepatopankreas (atas) dibandingkan udang normal dengan hepatopankreas yang utuh (bawah). *Sumber gambar:* [Nusantics](https://nusantics.com/en/blog/tackling-pira-and-pirb-toxins-causing-ahpnd-in-shrimp)](https://d1a39lp0neyni6.cloudfront.net/media/blog/image/528/dbea2264dba27cf099377a18a479d3b5.webp){.content-image fig-alt="Perbandingan hepatopankreas udang dengan AHPND dan udang normal" width="85%"}
...
::: download-row
[⬇ Raw reads — 10 sampel paired-end, .tar.gz](data/rnaseq-demo/raw_reads.tar.gz){.btn-download download="raw_reads.tar.gz"}

[⬇ Count matrix — gene_count_matrix.csv](data/rnaseq-demo/gene_count_matrix.csv){.btn-download download="gene_count_matrix.csv"}

[⬇ Metadata — metadata.csv](data/rnaseq-demo/metadata.csv){.btn-download download="metadata.csv"}
:::

Genom referensi demo (`demo_genome.fa`) beserta anotasinya (`demo_annotation.gtf`) tersedia di folder [docs/data/rnaseq-demo/ref](https://github.com/bowo1698/Bioinfonote/tree/main/docs/data/rnaseq-demo/ref). Seluruh pipeline alignment hingga matriks hitungan pada data demo dapat dijalankan sekaligus lewat skrip [`code/run-rnaseq-pipeline.sh`](https://github.com/bowo1698/Bioinfonote/blob/main/code/run-rnaseq-pipeline.sh) yang menjalankan persis perintah HISAT2 → StringTie → prepDE pada bagian-bagian berikut. Hasil akhirnya adalah `gene_count_matrix.csv` yang dipakai pada analisis DESeq2 di bagian akhir tutorial.
